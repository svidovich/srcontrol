...
class TestMetricParsers(unittest.TestCase):
    ...
    def test_parse_metric_key_value_with_bad_path(self):
        ...
        with self.assertRaises(FileNotFoundError):
            _ = metric.parse_metric_key_value(
                cgroup_path=bad_memory_cgroup_path,
                metric_name='memory.limit_in_bytes')

    def test_parse_metric_pairs_with_bad_path(self):
        bad_path = '/sys/fd/cgroup'
        bad_memory_cgroup_path = os.path.join(bad_path,
                                              'memory.limit_in_bytes')
        with self.assertRaises(FileNotFoundError):
            _ = metric.parse_metric_pairs(cgroup_path=bad_memory_cgroup_path,
                                          metric_name='memory.stat')
    ...
    def test_parse_metric_key_value_with_good_inputs(self):
        ...
        output = metric.parse_metric_key_value(cgroup_path=memory_cgroup_path,
    ...
    def test_parse_metric_pairs_with_good_inputs(self):
        ...
        output = metric.parse_metric_pairs(cgroup_path=memory_cgroup_path,
                                           metric_name=test_metric_name)
    ...
    def test_detect_metric_type_with_key_value(self):
        memory_cgroup_path = os.path.join(CGROUP_BASE_DIR, 'memory')
        test_metric_name = 'memory.limit_in_bytes'
        with open(os.path.join(memory_cgroup_path,
                               test_metric_name)) as file_handle:
    ...
    def test_detect_metric_type_with_chain(self):
        ...
        with open(os.path.join(cpu_cgroup_path,
                               test_metric_name)) as file_handle:
            metric_type = metric.detect_metric_type(file_handle)
